...
public class SshConnectionManager implements SshKnownHostsManager {
	...
	public int connect(ConnectionParams connectionParams) throws CancelConnectionException {
        ...
        //        int port = 0;
		try {
            //session = jsch.getSession(connectionParams.sshUserName, connectionParams.sshHostName, connectionParams.getSshPortNumber());
            session = jsch.getSession(connectionParams.getSshUserName(), connectionParams.getHostName(), 22);
            //            UserInfo ui = new SwingSshUserInfo(parentWindow);
            //            session.setUserInfo(ui);
            session.setPassword(connectionParams.getSshPassword());
        ...
        //return port;
        return connectionParams.getPortNumber();
	}

    private void sendStartX11vnc() throws JSchException {
        try {
            Channel channel = session.openChannel("exec");
            ((ChannelExec) channel).setCommand("x11vnc");
