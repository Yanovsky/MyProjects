...
public class ConnectionsHistory implements Model {
    ...
    public static final String NODE_SSH_PASSWORD = "sshPassword";
    ...
    private void retrieve() {
        ...
        try {
			...
			for (String orderNum : orderNums) {
                ...
                ConnectionParams cp = new ConnectionParams(hostName, node.getInt(NODE_PORT_NUMBER, 0), node.get(NODE_SSH_USER_NAME, ""), node.get(NODE_SSH_PASSWORD, "")
                );
    ...
    private void serializeConnectionParams(Preferences node, ConnectionParams connectionParams) {
        ...
        node.put(NODE_SSH_PASSWORD, connectionParams.sshPassword != null ? connectionParams.sshPassword : "324012");
 ...
 orig.getPortNumber() == cp.getPortNumber()) {
            ...
            if (orig.hostName.equals(cp.hostName) &&
                    ...
                    compareTextFields(orig.sshPassword, res.sshPassword, cp.sshPassword)) {
                res = cp;
            }
        }
        return res;
